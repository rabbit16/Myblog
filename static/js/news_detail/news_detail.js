$(function () {
  // 未登录提示框
  let $loginComment = $('.please-login-comment input'); //获取未登录提示框的输入框
    let $send_comment = $('.logged-comment .comment-btn');//获取按钮
  ...
  $loginComment.click(function () {
    ...
    $.ajax({
      url: "/article/" + $(".please-login-comment").attr('news-id') + "/comments/",
      type: "POST",
      contentType: "application/json; charset=utf-8",
      dataType: "json",
    })
